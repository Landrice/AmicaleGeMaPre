...
public class CatagoryGetway {
    ...
    public void view(Catagory catagory, String sigle) {
        con = dbCon.geConnection();
        try {
            con = dbCon.geConnection();
            ...
            while (rs.next()) {
                ...
                catagory.creatorName = sql.getNameR(catagory.creatorId, catagory.catagoryName, ""+sigle+"_utilisateur",sigle);
    ...
    public void selectedView(Catagory catagory, String sigle) {
        con = dbCon.geConnection();
        try {
            ...
            while (rs.next()) {
                ...
                catagory.brandId = rs.getString(4);
                catagory.supplyerId = rs.getString(5);
                catagory.brandName = sql.getNameR(catagory.brandId, catagory.brandName, "marque",sigle);
                catagory.supplyerName = sql.getNameR(catagory.supplyerId, catagory.supplyerName, "fournisseur",sigle);
            }
            pst.close();
            con.close();
            rs.close();
    ...
    public void searchView(Catagory catagory, String sigle) {
        con = dbCon.geConnection();
    ...
    public void update(Catagory catagory) {
        ...
        try {
            pst = con.prepareStatement("update "+db+".ategorie set NomCategory=? , DescriptionCategory=?,MarqueId=?,FournisseurId=? where Id=?");
            pst.setString(1, catagory.catagoryName);
    ...
    public boolean isNotUse(Catagory catagory, String sigle) {
        con = dbCon.geConnection();
        boolean isNotUse = false;
        try {
            pst = con.prepareCall("select * from "+db+"."+sigle+"_produits where CategorieId=?");
            pst.setString(1, catagory.id);
            rs = pst.executeQuery();
            while (rs.next()) {
